...
const Congress = ({ formik }: Props) => {
  ...
  const { data, loading, error } = useQuery(GET_BALLOT, {
    ...
    onCompleted: (data) => {
      ...
      //console.info('detail: ', detail.length);
      //console.info('ballotByLevel: ', data?.ballotByLevel);
      if (detail.length === 0) {
        ...
        <Card sx={{ p: 3 }}>
          ...
          >
            {data.ballotByLevel.map((item: any, index: number) => {
              ...
              if (detailItemIndex >= 0) {
                vote = toSafeNumber(values.detail[detailItemIndex]?.votes);
              }

              return (
                <Grid key={`grid-item-${item.candidate_id}`} size="grow">
                  <Grid
                    container
                    size="grow"
                    direction="row"
